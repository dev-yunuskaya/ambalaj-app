...
const db = new sqlite3.Database("./ambalaj.db", (err) => {
    if (err) console.error(err);
        ...
        db.run(
            `CREATE TABLE IF NOT EXISTS prices (user_id INTEGER, key TEXT, value TEXT, PRIMARY KEY(user_id, key))`
        );
    }
});
...
app.get("/api/prices", authenticate, (req, res) => {
    db.all(
        "SELECT key, value FROM prices WHERE user_id = ?",
        [req.user.id],
        (err, rows) => {
            const prices = {};
...
app.post("/api/prices", authenticate, (req, res) => {
    ...
    db.serialize(() => {
        ...
        Object.entries(prices).forEach(([key, value]) => {
            db.run(
                "INSERT INTO prices (user_id, key, value) VALUES (?, ?, ?)",
